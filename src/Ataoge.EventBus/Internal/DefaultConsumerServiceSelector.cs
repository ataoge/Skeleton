...
namespace Ataoge.EventBus.Internal
{
    ...
    internal class DefaultConsumerServiceSelector : IConsumerServiceSelector
    {
        ...
        /// </summary>
        public DefaultConsumerServiceSelector(IServiceProvider serviceProvider, ConfigOptions capOptions)
        {
            ...
            IServiceProvider provider)
        {
            ...
            using (var scoped = provider.CreateScope())
            {
                ...
                var consumerServices = scopedProvider.GetServices<ISubscriber>();
                foreach (var service in consumerServices)
                {
                    var typeInfo = service.GetType().GetTypeInfo();
                    if (!typeof(ISubscriber).GetTypeInfo().IsAssignableFrom(typeInfo))
                    {
                        continue;
                    }
        ...
        private IEnumerable<ConsumerExecutorDescriptor> FindConsumersFromControllerTypes()
        {
            ...
            foreach (var type in types)
            {
                ...
                if (CommonHelper.IsController(typeInfo))
                {
                    executorDescriptorList.AddRange(GetTopicAttributesDescription(typeInfo));
                }
            }
